...
CREATE OR REPLACE PACKAGE BODY G_WAP_ZTE AS
  ...
  --
    ...
    SELECT  to_date((SUBSTR(SUBSTR(nombre_csv,INSTR(nombre_csv,'_',-1)+1,8),7,2)||
            ...
            '.'||
            SUBSTR(SUBSTR(nombre_csv,INSTR(nombre_csv,'_',-1)+1,8),1,4)),'dd.mm.yyyy')-1 fecha,
            GATEWAY,
    ...
    LOOP
      ...
      BEGIN
        FORALL indice in v_wap_gtw_srv_tab.first .. v_wap_gtw_srv_tab.last SAVE EXCEPTIONS
          insert into  WAP_GATEWAY_SERVICE_ZTE_RAW (FECHA,
                                                    WAP_BROWSER_REQUEST_NUMBER,
                                                    WAP_BROWSER_REQ_SUCCESS_RATIO,
                                                    HTTP_BROWSER_REQUEST_NUMBER,
                                                    HTTP_BROWSER_REQ_SUCCESS_RATIO,
                                                    ...
                                                    SP_DELAY,
                                                    SERVICE_DELAY)
          VALUES  (v_wap_gtw_srv_tab(indice).FECHA,
          ...
          WHEN OTHERS THEN
            ...
            LOOP
              ...
              g_error_log_new.P_LOG_ERROR('F_WAP_GTW_SERVICE_ZTE_RAW_INS',
                                          L_ERRNO,
                                          L_MSG,
                                          'FECHA => '                           ||v_wap_gtw_srv_tab(indice).FECHA||
  ...
  BEGIN
    --
    G_ERROR_LOG_NEW.P_LOG_ERROR('P_WAP_GATEWAY_SERVICE_ZTE_DAY',0,'NO ERROR','COMIENZO DE SUMARIZACION DAY P_WAP_GATEWAY_SERVICE_ZTE_DAY('||P_FECHA||')');
    P_WAP_GATEWAY_SERVICE_ZTE_DAY(P_FECHA);
    G_ERROR_LOG_NEW.P_LOG_ERROR('P_WAP_GATEWAY_SERVICE_ZTE_DAY',0,'NO ERROR','FIN DE SUMARIZACION DAY');
    --
    G_ERROR_LOG_NEW.P_LOG_ERROR('P_WAP_GATEWAY_SERVICE_ZTE_BH',0,'NO ERROR','COMIENZO CALCULO BH P_WAP_GATEWAY_SERVICE_ZTE_BH('||P_FECHA||')');
    P_WAP_GATEWAY_SERVICE_ZTE_BH(P_FECHA);
    G_ERROR_LOG_NEW.P_LOG_ERROR('P_WAP_GATEWAY_SERVICE_ZTE_BH',0,'NO ERROR','FIN CALCULO BH');
    --
    ...
    IF (TRIM(V_DIA) = 'SUNDAY') OR (TRIM(V_DIA) = 'DOMINGO') THEN
      G_ERROR_LOG_NEW.P_LOG_ERROR('P_WAP_GATEWAY_SERVICE_ZTE_IBHW',0,'NO ERROR','COMIENZO DE SUMARIZACION IBHW P_FECHA_DOMINGO => '||
                                  to_char(to_date(P_FECHA,'DD.MM.YYYY')-7,'DD.MM.YYYY')||
                                  ' P_FECHA_SABADO => '||TO_CHAR(TO_DATE(P_FECHA,'DD.MM.YYYY')-1,'DD.MM.YYYY'));
      --
      P_WAP_GATEWAY_SERVICE_ZTE_IBHW(to_char(to_date(P_FECHA,'DD.MM.YYYY')-7,'DD.MM.YYYY'),TO_CHAR(TO_DATE(P_FECHA,'DD.MM.YYYY')-1,'DD.MM.YYYY'));
      --
      G_ERROR_LOG_NEW.P_LOG_ERROR('P_WAP_GATEWAY_SERVICE_ZTE_IBHW',0,'NO ERROR','FIN DE SUMARIZACION IBHW');
    END IF;
